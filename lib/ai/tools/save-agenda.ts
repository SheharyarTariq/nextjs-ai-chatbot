...
export const createSaveAgendaTool = ({
  ...
  tool({
    description:
      "Save the athlete's training agenda to the database after completing the onboarding process AND generating the initial weekly plan. This should be called once all onboarding information is collected AND the first week's training plan with all session details (exercise, meals, sleep) has been created. NOTE: The user's name, gender, and age are already available from their profile - use those values when saving userData.\n\nIMPORTANT - EVENT AWARENESS: Before generating the weekly plan, you MUST:\n1. Check if the user has any existing joined events during the 12-week training period.\n2. For each day that has an event:\n   - If the event type matches the planned training type (e.g., both Run, both Yoga): DO NOT create a training session for that day. The event replaces the training.\n   - If the event type differs from the planned training: REDUCE the training load for that day (no double sessions, lower intensity/duration). The user needs to accommodate both the event and some training.\n3. MAINTAIN THE 12-WEEK GOAL: Redistribute any skipped or reduced training across other days or weeks to ensure the goal remains achievable.\n4. Explain to the user how you've adjusted their plan around their existing events.",
